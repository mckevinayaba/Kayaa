...
function RateForm({ placeId, onDone }: { placeId: string; onDone: () => void }) {
  ...
  return (
    <div style={{ marginTop: '12px', padding: '12px', background: 'rgba(255,255,255,0.04)', borderRadius: '10px' }}>
      <div style={{ fontFamily: 'Inter, sans-serif', fontSize: '13px', color: 'rgba(255,255,255,0.6)', marginBottom: '10px' }}>
      ...
      {score > 0 && (
        <div style={{ fontFamily: 'Inter, sans-serif', fontSize: '12px', color: ratingColor(score), marginBottom: '10px', fontWeight: 600 }}>
          {ratingLabel(score)}
        </div>
      )}
      <button
  ...
  if (!showDetails) {
    return (
      <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
        ...
        <span style={{ fontFamily: 'Inter, sans-serif', fontSize: '14px', fontWeight: 700, color }}>
          {rating.toFixed(1)}
        </span>
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '12px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
          ...
          <span style={{ fontFamily: 'Inter, sans-serif', fontWeight: 800, fontSize: '22px', color }}>
            {rating.toFixed(1)}
          </span>
        </div>
      </div>
